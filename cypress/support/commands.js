...
Cypress.Commands.add('login_hr',()=>{
   
    cy.get('#username').type("dinesh.karki@infodevelopers.com.np")
    cy.get('#password').type("kathmandu@123")
...
Cypress.Commands.add('kcLogin', (username, password) => {
  const kcRoot = 'http://192.168.50.140:9001';
  const kcRealm = 'InfoDevelopers';
  const kcClient = 'hr-utility-frontend';
  const kcRedirectUri = 'http://192.168.50.172:3000/master/meeting-hall';
